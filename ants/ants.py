...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect):		#insert a non-container ant
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant):		#insert a container ant
                    ant = self.ant
                    self.ant = insect
                    self.ant.contain_ant(ant)
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            import ants
            if isinstance(insect, ants.QueenAnt):
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        if self.spookied and self.spook_time:
...
class ScubaThrower(ThrowerAnt):
    """A ThrowerAnt that can survive on water."""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_imposter:
        	...
        	while current_place.exit:
        		buff_ant = current_place.exit.ant
        		if buff_ant and buff_ant not in self.doubled:
        			buff_ant.damage = buff_ant.damage * 2
        			self.doubled.append(buff_ant)
        		if isinstance(buff_ant, BodyguardAnt) and buff_ant.contained_ant and \
        		buff_ant.contained_ant not in self.doubled:
        			buff_ant.contained_ant.damage = buff_ant.contained_ant.damage * 2
        			self.doubled.append(buff_ant.contained_ant)
    ...
    def reduce_armor(self, amount):
        ...
        if self.place.ants is not None and not self.place.ant.is_imposter:
        	bees_win()
        elif self.is_impotser:
        	self.place.remove_insect(self)
        """
        Insect.reduce_armor(self, amount)
        if self.armor <= 0:
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    init_action = bee.action
    new_action = effect(bee.action, bee)
    
    def action(colony):
        nonlocal duration
        if duration == 0:
            return init_action(colony)
        else:
            duration -= 1
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if not self.spookied and target:
